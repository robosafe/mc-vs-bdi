...
# in the file: no_automata states . x . x . x . x . x . x. x .. no_variables . transitions . [repeat]
	...
	all_lines.pop(0) #Eliminate first . of the sequence
	while True:
		if re.search('(\d+)',all_lines[0]) and re.search('(\d+)',all_lines[1]) and re.search('(\d+)',all_lines[2]) and re.search('.',all_lines[3]):
			all_lines.pop(0)
			all_lines.pop(0)
			all_lines.pop(0)
			all_lines.pop(0)
			...
			break
	all_lines.pop(0) #Eliminate double . of the sequence
...
#	print state_0
	...
	while len(all_lines)>2:
		...
		all_lines.pop(0) #Eliminate first . of sequence
		while True:
			if re.search('(\d+)',all_lines[0]) and re.search('(\d+)',all_lines[1]) and re.search('(\d+)',all_lines[2]) and re.search('.',all_lines[3]):
				all_lines.pop(0)
				all_lines.pop(0)
		...
		count = 0
		if len(all_lines) > no_automata:
			for i in range(0,no_automata):
				if re.search("(\d+\s\d+)", all_lines[i]):
					#print all_lines[i]
					count = count + 1
					transitions_1.append(all_lines[i])
		else:
		...
		for i in range(0,len(transitions_1)):
			...
			particular_tran = all_tran_aut[int(automaton_t[1])] #Finding particular transition
