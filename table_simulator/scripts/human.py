...
def main(name_file,xx):
	...
	global instructions
	global data
	for num,command in enumerate(open(os.getcwd()+'/src/table_simulator/scripts/mutation_abstract_tests/'+name_file+'.txt','r')): 
#	for num,command in enumerate(open(os.getcwd()+'/src/table_simulator/scripts/abstract_tests/'+name_file+'.txt','r')): 
#	for num,command in enumerate(open(os.getcwd()+'/src/table_simulator/scripts/pseudorandom/'+name_file+'.txt','r')):
#	for num,command in enumerate(open(os.getcwd()+'/src/table_simulator/scripts/mbt/'+name_file+'.txt','r')):
#		print command
		if re.search("tell\s*leg",command): #if the command is to send a signal
			instructions.append('sendA1')
		elif re.search("tell\s*humanReady",command):
			instructions.append('sendA2')
		elif re.search("set_param\s*location\s*=\s*1",command): #if the command is to set the value of a variable
			instructions.append('location1') 
		elif re.search("set_param\s*location\s*=\s*0",command):
			instructions.append('location0') 
		elif re.search("set_param\s*pressure\s*=\s*1",command):
			instructions.append('pressure1')
		elif re.search("set_param\s*pressure\s*=\s*0",command):
			instructions.append('pressure0')
		elif re.search("set_param\s*gaze\s*=\s*1",command):
			instructions.append('gaze1')
		elif re.search("set_param\s*gaze\s*=\s*0",command):
