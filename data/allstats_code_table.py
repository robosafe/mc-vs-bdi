...
for jj in range(10,51):
	temp_bdi=[]
	temp_var_bdi=[]
	for num,line in enumerate(open(os.getcwd()+'/data_diffseeds_table_bdi/stats_'+str(jj)+'.txt','r')): 
	...
	temp_pr=[]
	temp_var_pr=[]
	for num,line in enumerate(open(os.getcwd()+'/data_diffseeds_table_pr/stats_'+str(jj)+'.txt','r')): 
		if re.search("Covered percentage",line):
			getdata = re.split("Covered percentage[:]",line)
			getdata = re.split("[%]",getdata[1])
	...
	temp_mb=[]
	temp_var_mb=[]
	for num,line in enumerate(open(os.getcwd()+'/data_diffseeds_table_mc/stats_'+str(jj)+'.txt','r')): 
		if re.search("Covered percentage",line):
			getdata = re.split("Covered percentage[:]",line)
			getdata = re.split("[%]",getdata[1])
...
#plt.savefig('allstatsa.pdf')
plt.savefig('allstatsa.eps')
"""
plt.figure(2)
print len(bdi_results1)
x1 = range(1,len(bdi_results1)+1)
doublemean_bdi = 0.0
doublemean_mc = 0.0
doublemean_pr = 0.0
mean_bdi = []
mean_mc = []
mean_pr = []
for jj in range(0,len(bdi_results1)):
...
plt.xlabel('Tests')
plt.axis([0, 6560, 0, 105])
plt.savefig('convergencea.eps')
