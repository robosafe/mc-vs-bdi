...
for num,line in enumerate(open(os.getcwd()+'/data_diffseeds_table_bdi/stats_10.txt','r')): 
...
for num,line in enumerate(open(os.getcwd()+'/data_diffseeds_table_pr/stats_10.txt','r')): 
	if re.search("Covered percentage",line):
		getdata = re.split("Covered percentage[:]",line)
		getdata = re.split("[%]",getdata[1])
...
for num,line in enumerate(open(os.getcwd()+'/data_diffseeds_table_mc/stats_10.txt','r')): 
	if re.search("Covered percentage",line):
		getdata = re.split("Covered percentage[:]",line)
		getdata = re.split("[%]",getdata[1])
...
plt.savefig('codecoveragea2.eps')
